...
var creatorList = ['@ZinxLord','@ZansLord','@takimtod','@ardacontol', '@dittaz','@agam','@zinx'];
...
var len = 15
        ...
        var randomTextNumber = random+randomlagi+'---------ZinxGanteng'+'ZINX--GANS';
...
router.get('/find', async (req, res, next) => {
    ...
    if (apikey != 'zinxteam') return res.json(loghandler.invalidKey)
...
router.get('/tiktod', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'zincteam') return res.json(loghandler.invalidKey)
...
router.get('/tiktod/stalk', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'zinxteam') return res.json(loghandler.invalidKey)
...
router.get('/nulis', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	 ...
	 if(!text) return res.json(loghandler.nottext)
		Nulis(text)
		 .then(hasil => {
			fetch(encodeURI(`https://api.imgbb.com/1/upload?expiration=120&key=fac4058dfcc464b8448e9c333619b760&image=${hasil}&name=${randomTextNumber}`))
                                .then(response => response.json())
                                .then(data => {
                ...
                body: `text_1=${text}&text_2=${text2}&login=OK`,
                    ...
                    if (!e) {
                        ...
                        $(".thumbnail").find("img").each(function() {
                            ...
                            fetch(encodeURI(`https://api.imgbb.com/1/upload?expiration=120&key=fac4058dfcc464b8448e9c333619b760&image=${result}&name=${randomTextNumber}`))
                                .then(response => response.json())
                                .then(data => {
                                    var urlnya = data.data.url,
                                        delete_url = data.data.delete_url;
